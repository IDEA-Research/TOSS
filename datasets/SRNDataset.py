...
class SRNDataModuleFromConfig(pl.LightningDataModule):
    ...
    def test_dataloader(self):
        return wds.WebLoader(SRNDataset(root_dir=self.root_dir, total_view=251, validation=False, test=True),
                          batch_size=self.batch_size, num_workers=self.num_workers, shuffle=False, caption=self.caption, pose_enc=self.pose_enc)
...
class SRNDataset(Dataset):
    def __init__(self,
        ...
        if caption == 'random':
            if validation:
                paths_file = f"/comp_robot/mm_generative/data/nerf/srn_cars/val_caption_allviews.json"
            elif test:
                if test_sub:
                    paths_file = "/comp_robot/mm_generative/data/nerf/srn_cars/test_sub_caption_allviews.json"
                else:
                    paths_file = "/comp_robot/mm_generative/data/nerf/srn_cars/test_caption_allviews.json"
            else:
                paths_file = "/comp_robot/mm_generative/data/nerf/srn_cars/train_caption_allviews.json"
        elif caption == "rerank":
            if validation:
                paths_file = "/comp_robot/mm_generative/data/nerf/srn_cars/val_caption_clip_rerank.json"
            elif test:
                if test_sub:
                    paths_file = "/comp_robot/mm_generative/data/nerf/srn_cars/test_sub_caption_clip_rerank.json"
                else:
                    paths_file = "/comp_robot/mm_generative/data/nerf/srn_cars/test_caption_clip_rerank.json"
            else:
                paths_file = "/comp_robot/mm_generative/data/nerf/srn_cars/train_caption_clip_rerank.json"
        else:
            raise(f"caption type {caption} not supported")
        
        if validation:
            self.root_dir = os.path.join(self.root_dir, 'cars_val')
        elif test:
            if test_sub:
                self.root_dir = os.path.join(self.root_dir, 'cars_test_sub')
            else:
                self.root_dir = os.path.join(self.root_dir, 'cars_test')
        else:
            self.root_dir = os.path.join(self.root_dir, 'cars_train')
...
class SRNTestDataset(SRNDataset):
    def __init__(self,
        ...
        super().__init__(
            ...
            test=True, caption=caption, test_sub=test_sub, pose_enc=pose_enc
        )
