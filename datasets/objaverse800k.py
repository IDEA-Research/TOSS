...
class ObjaverseDataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, root_dir, batch_size, total_view, train=None, validation=None,
                 test=None, num_workers=4, caption="rerank", pose_enc="freq", **kwargs):
        super().__init__()
    ...
    def test_dataloader(self):
        return wds.WebLoader(ObjaverseData(root_dir=self.root_dir, total_view=self.total_view, validation=self.validation, caption=self.caption, pose_enc=self.pose_enc),\
...
class ObjaverseData(Dataset):
    def __init__(self,
        ...
        if caption == "recaption":    
            paths_file = f"{root_dir}/valid_paths_recap.json"
        elif caption == 'random':
            paths_file = f"{root_dir}/valid_paths_4views.json"
        elif caption == "rerank":
            paths_file = f"{root_dir}/valid_paths_clip_rerank.json"
        elif caption == "cap3d":
            paths_file = f"{root_dir}/valid_paths_clip_rerank_Cap3D.json"
        else:
            paths_file = f"{root_dir}/valid_paths.json"
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            filename = os.path.join(self.root_dir, '2b753ef48a764a06b75f8acce125165a') # this one we know is valid
            target_im = self.process_im(self.load_im(os.path.join(filename, '%03d.png' % index_target), color))
            cond_im = self.process_im(self.load_im(os.path.join(filename, '%03d.png' % index_cond), color))
            target_RT = np.load(os.path.join(filename, '%03d.npy' % index_target))
            cond_RT = np.load(os.path.join(filename, '%03d.npy' % index_cond))
